...
opt.TBEFORE = 50.0; % time before to cut
opt.TAFTER = 150.0; % time after p to cut
opt.TAPERW = 0.05; % proportion of seis to taper
opt.taperTypeFlag = 1;
opt.FLP = 2.0; % low pass
opt.FHP = 0.05; % high pass
...
rfOpt.ITERMAX = 50; %  max number of iterations in deconvolution
...
enzfiles = getThreeCompFilenames( basedir , 'E', 'N', 'Z');
...
%for i =1:1,
  ...
  % Note getArrTime is not perfect. It assumes a certain set of arrival
  % times mapped to USERN sac header variables. Different researchers store
  % arrival times in different header locations and thus this function is not
  % sensitive to that.
  ...
  [rftime, rfseis, rfhdr, ierr] = processRFiter(rseis, zseis, hdr, rfOpt , false);

  if ierr == 1
      %error('Error, rms array empty.');
      disp('Error, rms array empty.');
      continue
  end
...
% staDIR(find(isspace(staDIR)))=[];
temp_l=length(staDIR);
if (temp_l==9)
    staDIR=[staDIR(1),staDIR(2),staDIR(4),...
        staDIR(5),staDIR(6),staDIR(7),staDIR(9)];
elseif (temp_l==10)
    staDIR=[staDIR(1),staDIR(2),staDIR(4),...
        staDIR(5),staDIR(6),staDIR(7),staDIR(8),staDIR(10)];
end
...
temp1=deblank(strtrim(rfhdr.station.knetwk));
temp2=deblank(strtrim(rfhdr.station.kstnm));
